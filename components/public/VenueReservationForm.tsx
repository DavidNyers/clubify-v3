...
export default function VenueReservationForm({ venueId, venueType, venueName, source = 'app', venueZones, showZones = true, onClose }: Props) {
  ...
  const allHours = [17,18,19,20,21,22,23]
      ...
      >
        ...
        <button 
          onClick={onClose ? onClose : () => {
            ...
            if (slug && window.location.pathname.includes('/reserve/')) {
              window.location.href = `/${venueType}s/${slug}`
            } else {
    ...
    <div style={{ width: '100%', maxWidth: 600, margin: '0 auto' }}>
      <form onSubmit={(e) => handleSubmit(e)} style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            <div className="glass" style={{ background: 'rgba(24, 24, 27, 0.4)', border: '1px solid rgba(255, 255, 255, 0.05)', borderRadius: 32, padding: 32, backdropFilter: 'blur(20px)' }}>
                      ...
                      {activePicker === 'time' && (
                        ...
                        >
                           <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                              {allTimePossibleSlots
                                 .filter(t => getTimeStatus(t) === 'available')
                                 .map(t => {
              ...
              <div id="time-grid" style={{ marginTop: 8, padding: '24px', background: 'rgba(0,0,0,0.2)', borderRadius: 24, border: '1px solid rgba(255,255,255,0.03)', maxWidth: 500, margin: '8px auto 0 auto' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, justifyContent: 'center' }}>
                  ...
                  {allTimePossibleSlots.filter(t => getTimeStatus(t) === 'available').length === 0 && (
                    <p style={{ color: '#71717a', fontSize: '0.9rem', textAlign: 'center', margin: '20px 0' }}>Keine freien Zeiten für diesen Tag verfügbar.</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <button 
              ...
              style={{ 
                ...
                opacity: (!formData.time || !formData.date) ? 0.6 : 1, transition: 'all 0.2s'
              }}
        ...
        {step === 2 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            {/* Selection Summary */}
            <div style={{ background: 'rgba(139, 92, 246, 0.05)', border: '1px solid rgba(139, 92, 246, 0.15)', borderRadius: 20, padding: '16px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div style={{ fontSize: '0.8rem', color: '#a1a1aa', marginBottom: 2 }}>Ausgewählt:</div>
                <div style={{ fontSize: '0.95rem', fontWeight: 800, color: 'white' }}>
                  {formData.guests} {formData.guests === 1 ? 'Person' : 'Personen'} am {formData.date.split('-').reverse().join('.')} um {formData.time} Uhr
                </div>
              </div>
              <button 
                type="button" 
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <button 
                ...
                onClick={() => setStep(1)}
                style={{ 
                  flex: 1, padding: 20, borderRadius: 24, border: '1px solid rgba(255,255,255,0.1)', 
              ...
              <button 
                ...
                disabled={loading}
                style={{ 
                  flex: 2, padding: 20, borderRadius: 24, border: 'none', 
                  background: 'linear-gradient(135deg, #8b5cf6, #ec4899)', color: 'white', 
                  fontSize: '1.1rem', fontWeight: 950, cursor: 'pointer', display: 'flex', 
                  alignItems: 'center', justifyContent: 'center', gap: 12,
                  boxShadow: '0 15px 35px -10px rgba(139, 92, 246, 0.4)',
