...
import { Calendar, CheckCircle, Loader2, Beer, User } from 'lucide-react'
import { respondToVenueRequest } from '@/lib/actions/venue/EventVerificationActions'
import { motion, AnimatePresence } from 'framer-motion'

export default function BarOwnerEventRequests() {
  ...
  async function fetchRequests() {
    ...
    const { data: myBars } = await supabase.from('bars').select('id, name').eq('owner_id', user.id)
    const barIds = myBars?.map(b => b.id) || []

    if (barIds.length > 0) {
      const { data: events } = await supabase
        .from('events')
        .select('*, users!events_manager_id_fkey(full_name, email), bars!events_bar_id_fkey(name)')
        .in('bar_id', barIds)
    ...
    <div style={{ padding: 32, maxWidth: 1000, margin: '0 auto' }}>
      <header style={{ marginBottom: 32 }}>
        <h1 style={{ fontSize: '1.8rem', fontWeight: 800, color: 'white', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 12 }}>
          <CheckCircle style={{ color: '#3b82f6' }} size={32} /> Eingehende Event-Anfragen
        </h1>
        <p style={{ color: '#71717a' }}>Externe Veranstalter, die deine Bars als offizielle Location nutzen möchten.</p>
      </header>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#e2e8f0', fontSize: '0.85rem' }}>
                        <Beer size={14} style={{ color: '#3b82f6' }} /> Gewünschte Bar: <strong>{req.bars?.name}</strong>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#e2e8f0', fontSize: '0.85rem' }}>
