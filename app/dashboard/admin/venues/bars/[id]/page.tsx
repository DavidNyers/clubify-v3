...
import { GlassWater, Save, MapPin, Users, Loader2, ArrowLeft, ShieldAlert } from 'lucide-react'
import Link from 'next/link'
import { useParams, useRouter } from 'next/navigation'
import { updateBarAdmin } from '@/lib/actions/admin/AdminVenueActions'
...
export default function AdminBarDetailPage() {
  ...
  useEffect(() => {
    async function loadItem() {
      ...
      const { data } = await supabase.from('bars').select('*, users(id, full_name, email)').eq('id', barId).single()
      if (data) {
        setBar(data)
        setFormData({
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      await updateBarAdmin(bar.id, formData)
  ...
  if (!bar) return <div style={{ padding: 40, color: 'white' }}>Bar nicht gefunden.</div>
    ...
    <div style={{ padding: 32, maxWidth: 800, margin: '0 auto' }}>
      ...
      <header style={{ marginBottom: 32, display: 'flex', alignItems: 'flex-start', gap: 16 }}>
        <div style={{ width: 64, height: 64, borderRadius: 16, background: 'rgba(59, 130, 246, 0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <GlassWater style={{ color: '#3b82f6' }} size={32} />
        </div>
        <div>
          <h1 style={{ fontSize: '1.8rem', fontWeight: 800, color: 'white', marginBottom: 4 }}>Bar Bearbeiten: {bar.name}</h1>
          <p style={{ color: '#71717a', display: 'flex', gap: 8, alignItems: 'center' }}>
             Besitzer: <Link href={`/dashboard/admin/users/${bar.users?.id}`} style={{ color: '#3b82f6', textDecoration: 'none' }}>{bar.users?.full_name} ({bar.users?.email})</Link>
          </p>
        </div>
      </header>
      ...
      <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
        ...
        <div className="glass" style={{ background: 'rgba(24, 24, 27, 0.4)', borderRadius: 20, padding: 24, border: '1px solid #27272a', display: 'flex', flexDirection: 'column', gap: 16 }}>
           ...
           <div>
             <label style={{ display: 'block', fontSize: '0.85rem', color: '#71717a', marginBottom: 6 }}>Bar Name</label>
        ...
        <button 
          ...
          style={{ 
            background: 'linear-gradient(135deg, #3b82f6, #ec4899)', border: 'none', padding: 16, borderRadius: 16,
